# Better version of twitter spider. Maintains friends relationships
import urllib
import twurl
import json
import sqlite3
...
while True:
    ...
    for u in js['users']:
        ...
        try:
            ...
            cur.execute('''INSERT OR IGNORE INTO People(name,retrieved) VALUES (?,0)''',(friend,))  #ignoring duplicte entries
            conn.commit()
            if cur.rowcount !=1:
            ...
            countnew=countnew+1
        cur.execute('INSERT OR IGNORE INTO Follows(from_id,to_id) VALUES (?,?)',(id,friend_id)) #ignoring duplicate entries
